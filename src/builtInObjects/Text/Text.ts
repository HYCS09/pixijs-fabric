...
import { FILL, LTR, RTL, STROKE } from '../../constants';
import type {
...
import type { TextStyleArray } from './support';
import {
  cache,
...
type PathWithSegments = Path & {
  segmentsInfo?: ReturnType<typeof getPathSegmentsInfo>;
};
  ...
  implements UniqueTextProps
{
  ...
  _getCacheCanvasDimensions(): TCacheCanvasDimensions {
    ...
    tempDim.width = dims.width + fontSize * dims.zoomX;
    tempDim.height = dims.height + fontSize * dims.zoomY;
    tempDim.x = dims.x;
    tempDim.y = dims.y;
    tempDim.zoomX = dims.zoomX;
    tempDim.zoomY = dims.zoomY;
  ...
  >(propertiesToInclude: K[] = []): Pick<T, K> & SProps {
    const base = {
      ...pick(
        ...
        Array.from(additionalProps) as string[],
      ),
      ...pick(this as Record<string, any>, propertiesToInclude as string[]),
    };
  ...
  >(object: T) {
    const path = object.path
      ? object.path instanceof Path
        ? object.path
        : new Path((object.path as any).path || [], object.path)
      : undefined;
    return new this(object.text, {
      ...object,
      path,
